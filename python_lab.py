...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [ (x,y,z) for x in S for y in S for z in S if x+y+z==0][1:]
...
first_of_tuples_list = [ (x,y,z) for x in S for y in S for z in S if x+y+z==0 and x != 0 and y != 0 ][0]
...
list_sum_zip = [ t[0]+t[1] for t in zip(A,B) ]
